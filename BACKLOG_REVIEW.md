...
**Статус на v0.6.0 (следующий P1-пакет по решению владельца проекта
«делай это»):** #15 (`PRICING_CHANGELOG.md`), #37 (`ARCHITECTURE.md`), #4
(`CostTracker.pricing_overrides`), #5 (`report --format csv`), #19 (строка
ICP в README), #31 (`llmledger validate`), #36 (`examples/*.py` обновлены
под `dashboard`/`--since`/`--until`, попутно найден и исправлен реальный
баг — `full_pipeline.py` не распаковывал новый кортеж `train()` из v0.5.0)
и #6 (`CONTRIBUTING.md`, включающий правило #8 про скриншоты) реализованы
полностью, см. `CHANGELOG.md`. Оставшиеся P1-пункты (#3b, #7, #8) не
затронуты этим пакетом.
...
| **P1** | 15 | ~~Нет истории изменений самого `pricing.json`~~ | Backend/Core Engineer, Technical Writer/Docs | **Готово (v0.6.0)** |
| **P1** | 36 | ~~`examples/*.py` никогда не обновлялись под `dashboard`/`--since`/`--until`~~ | Technical Writer/Docs | **Готово (v0.6.0)** |
| **P1** | 37 | ~~Нет `ARCHITECTURE.md`, фиксирующего правило zero-dependency-ядра/extras-only~~ | CTO | **Готово (v0.6.0)** |
| **P1** | 19 | ~~Явная строка ICP в README~~ | Директор по продукту, Marketing/DevRel | **Готово (v0.6.0)** |
| **P1** | 4 | ~~Точечные переопределения `--pricing-file`~~ | Backend/Core Engineer | **Готово (v0.6.0)** |
| **P1** | 5 | ~~CSV/табличный экспорт (`report --format csv`)~~ | Backend/Core Engineer | **Готово (v0.6.0)** |
| **P1** | 6 | ~~CONTRIBUTING.md~~ | Technical Writer/Docs, Директор по продукту | **Готово (v0.6.0)** |
| **P1** | 31 | ~~Команда `llmledger validate`~~ | Backend/Core Engineer | **Готово (v0.6.0)** |
